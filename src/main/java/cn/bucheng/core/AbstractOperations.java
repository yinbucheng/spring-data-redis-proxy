...
public abstract class AbstractOperations<K, V> {
    ...
    @Nullable
    public <T> T execute(RedisCallback<T> callback, boolean exposeConnection) {
   ...
   public  byte[] rawKey(Object key) {
    ...
    public Set<TypedTuple<V>> deserializeTupleValues(Collection<Tuple> rawValues) {
        if (rawValues == null) {
            return null;
        }
    ...
    public TypedTuple<V> deserializeTuple(Tuple tuple) {
   ...
   public String deserializeString(byte[] value) {
     ...
     * @since 1.8
     */
    public GeoResults<GeoLocation<V>> deserializeGeoResults(GeoResults<GeoLocation<byte[]>> source) {
