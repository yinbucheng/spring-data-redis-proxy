...
import cn.bucheng.core.AbstractOperations;
...
public class DefaultValueOperations<K, V> extends AbstractOperations<K, V> implements ValueOperations<K, V> {
    ...
    public V getAndSet(K key, V newValue) {
        log.info("getAndSet key:{} ,newValue:{}", key, newValue);
    ...
    public Long increment(K key, long delta) {
        log.info("increment key:{} delta:{}", key, delta);
        byte[] rawKey = rawKey(key);
    ...
    public Long decrement(K key, long delta) {
        log.info("decrement key:{} delta:{}", key, delta);
        byte[] rawKey = rawKey(key);
